...
export default function PrivateChatModal({ userId, recipientName, onClose, currentUser,  verifiedUsername }: ChatProps) {
  ...
  const fetchMessages = async () => {
    ...
    try {
      const res = await axios.get(`https://radfinalprojectbackend-production.up.railway.app/api/chat/${userId}`, { headers });
  ...
  const handleSendSubmit = async (e?: React.FormEvent, overrideAttachments?: string[]) => {
    ...
    try {
      const res = await axios.post(
        'https://radfinalprojectbackend-production.up.railway.app/api/chat',
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const res = await axios.post('https://radfinalprojectbackend-production.up.railway.app/api/chat/upload', formData, {
  ...
  const handleUpdateMessage = async (msgId: string) => {
    ...
    try {
      const res = await axios.put(`https://radfinalprojectbackend-production.up.railway.app/api/chat/${msgId}`, { content: editText }, { headers });
  ...
  const executeDelete = async (msgId: string) => {
    try {
      const res = await axios.delete(`https://radfinalprojectbackend-production.up.railway.app/api/chat/${msgId}`, { headers });
      setMessages(messages.map(m => m._id === msgId ? res.data : m));
