...
export default function PrivateChatModal({
  ...
  const fetchMessages = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/chat/${userId}`, { headers });
  ...
  const handleSendSubmit = async (
    ...
    if (!userId || userId === "undefined") {
      toast.error(
        "Cannot send message: Invalid recipient profile reference.",
        toastOptions,
      );
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xl">
      <div className="flex flex-col h-[620px] w-full max-w-xl bg-zinc-950 border border-white/10 rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="flex-grow p-6 overflow-y-auto space-y-6 bg-zinc-950 scrollbar-thin scrollbar-thumb-zinc-700">
          {messages.map((msg) => {
            ...
            const isTargetRecipient =
              msg.receiver === userId || msg.sender === userId;
              ...
              >
                <div
                  onClick={() =>
                    !msg.isDeleted &&
                    isMe &&
                    setActiveMenuId(isMenuOpen ? null : msg._id)
                  }
                          ...
                          return isImage ? (
                            ...
                            >
                              <img
                                src={url}
                                alt="attachment"
                                className="rounded-2xl max-h-60 w-full object-cover border border-white/10"
                              />
                            </a>
                          ) : (
                            <a
                  ...
                  editingMessageId !== msg._id && (
                    <div className="flex items-center gap-1 mt-2 bg-zinc-900 border border-white/10 rounded-2xl p-1 shadow-xl">
                      ...
                      <button
                        ...
                        className="p-2.5 hover:bg-white/10 rounded-xl text-slate-400 hover:text-violet-400 transition"
                        title="Edit"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button
                        ...
                        className="p-2.5 hover:bg-white/10 rounded-xl text-slate-400 hover:text-red-500 transition"
                        title="Delete"
                      >
                        <Trash size={16} />
                      </button>
        ...
        {/* INPUT AREA */}
        <form
          onSubmit={(e) => handleSendSubmit(e)}
          className="border-t border-white/10 bg-zinc-900 p-4"
        >
          {showEmojiPicker && (
            ...
            <div className="flex flex-wrap gap-2 mb-3 px-1">
              {attachedUrls.map((url, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex items-center gap-2 bg-zinc-800 border border-white/10 pl-3 pr-2 py-1.5 rounded-2xl text-xs"
                  >
                    {isImg ? (
                      <img
                        src={url}
