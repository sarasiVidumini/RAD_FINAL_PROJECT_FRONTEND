...
export default function Upload() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.subjectCode) return toast.error("Please choose a subject");
    if (docType === 'paper' && !canUploadPapers) return toast.error("Unauthorized.");

    const MAX_FILE_SIZE = 10 * 1024 * 1024;
    for (const file of Array.from(files)) {
      if (file.size > MAX_FILE_SIZE) return toast.error(`"${file.name}" is too large.`);
    }
    ...
    // FIX: Explicitly get requestId from URL and append to payload
